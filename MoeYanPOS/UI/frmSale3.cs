    ...
    public partial class frmSale3 : Form
    {
        ...
        public frmSale3()
        {
            try
            {
                ...
                //long sysVoucherNo = Int64.Parse(voucherno + daltrans.GetTransitionID("Sale").ToString());
                ...
                int vouno3;
                vouno3 = 2;
                txtSystemVoucherNo.Text = Convert.ToString(Int64.Parse(txtSystemVoucherNo.Text) + vouno3);
                txtVoucherNo.Text = Convert.ToString(Int64.Parse(txtSystemVoucherNo.Text) + vouno3);
                //txtSystemVoucherNo.Text = LocationNo + voucherno + lblTransID.Text;
                ////Save Transition
                ...
                getlstcustomer = dalcustomer.GetCustomer("Auto","Cash",0);
        ...
        private void Save_TempTables()        
        {
            try
            {
                ...
                //{
                    ...
                    if (dgvSale.Rows[0].Cells[10].Value == null)
                    {
                        saveTampSale();
                    }
                    else if (dgvSale.Rows[0].Cells[10].Value.ToString() == "")
                    {
                        saveTampSale();
                    }
                    ...
                    bolsaledetail.Mtype = dgvSale.CurrentRow.Cells["colType"].Value.ToString();
        ...
        private void dgvSale_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                else if (dgvSale.CurrentCell.ColumnIndex == 5)
                {
                    ...
                    dgvSale.CurrentCell = dgvSale.Rows[dgvSale.CurrentCell.RowIndex].Cells[6];
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    MessageBox.Show("Total and Discount Amount are the same.Data Canno't Save.");
                //    dgvSale.Focus();
                //    return;
                //}
                    ...
                    if (isSaved == 1)
                    {
                        ...
                        for (int i = 0; i < dgvSale.Rows.Count; i++)
                        {
                            ...
                            if (dgvSale.Rows.Count > 0)
                            {
                                ...
                                else if (dgvSale.Rows[i].Cells["colSaleDetailID"].Value.ToString() == "" & dgvSale.Rows[i].Cells["colItemCode"].Value != "0000")
                                {
                                    BOLSale bolsaledetail = new BOLSale();
                                    bolsaledetail.SaleID = dalsale.GetMaxSaleID(0, txtSystemVoucherNo.Text);
                                    lblSaleID.Text = bolsaledetail.SaleID.ToString();
                ...
                else
                {
                    ...
                    if (isSaved == 1)
                    {
                        ...
                        for (int i = 0; i < dgvSale.Rows.Count - 1; i++)
                        {
                            ...
                            if (dgvSale.Rows.Count > 1)
                            {
                                if (dgvSale.Rows[i].Cells["colSaleDetailID"].Value != "")
                                {
                                    BOLSale bolsaledetail = new BOLSale();
                                    ...
                                    bolsaledetail.Description = dgvSale.Rows[i].Cells["colDescription"].Value.ToString();
                                    //bolsaledetail.Mtype = "Pcs";//dgvSale.Rows[i].Cells["colType"].Value.ToString();
                                    bolsaledetail.Mtype = dgvSale.Rows[i].Cells["colType"].Value.ToString();
                                    bolsaledetail.Qty = Int32.Parse(dgvSale.Rows[i].Cells["colQty"].Value.ToString());
                                    bolsaledetail.SalePrice = Decimal.Parse(dgvSale.Rows[i].Cells["colSalePrice"].Value.ToString());
                                    bolsaledetail.Total = Decimal.Parse(dgvSale.Rows[i].Cells["colTotal"].Value.ToString());
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    l_Report.Load(Application.StartupPath + @"\Report\RptSale.rpt");
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                if (cboPaymentType.Text == "Cash")
                {
                    ...
                    //this.UseWaitCursor = true;
                    ReportDocument l_Report = new ReportDocument();
                    // PrintDialog pd = new PrintDialog();  // add by ksaung
                    // PrintDocument pdoc = new PrintDocument(); // add by ksaung
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (lstsystem.Count > 0)
                        {
                            if (lstsystem[0].Printerslip != "")
                            {
                                ...
                                margins = l_Report.PrintOptions.PageMargins;
                                margins.bottomMargin = 1;
                                //margins.leftMargin = 750;
                                margins.leftMargin = 450; // for shop 103
                                margins.rightMargin = 750;
                                margins.topMargin = 1;

                                //PaperSize psize = new PaperSize("Custom", 100, 200);
                                // Apply the page margins.
                                l_Report.PrintOptions.ApplyPageMargins(margins);

                                l_Report.PrintOptions.PrinterName = lstsystem[0].Printerslip;
        ...
        private void txtCustomer_KeyDown(object sender, KeyEventArgs e)
       {
        ...
        private void txtPending1_Click(object sender, EventArgs e)
        {
            System.Windows.Forms.Form f1 = System.Windows.Forms.Application.OpenForms["frmSale"];
            if (((frmSale)f1) != null)
            {
                Application.OpenForms["frmSale"].BringToFront();
            }
            else
            {
        ...
        private void btnPreviewVoucher_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ReportDocument l_Report = new ReportDocument();
                DataSet ds = new DataSet();
                ds = dalsale.SelectSaleVoucher(0, 1, txtSystemVoucherNo.Text);
        ...
        private void btnA4Print_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet ds = new DataSet();
                //ds = dalsale.SelectSaleVoucher(long.Parse(lblSaleID.Text), 0, ""); remove by ksaung
                ds = dalsale.SelectSaleVoucherCredit(txtSystemVoucherNo.Text); //add by ksaung
